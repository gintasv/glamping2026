...
- The **Expenses tab** (`renderExpenses`) is a Venmo/Splitwise-style splitter. **All money is INTEGER CENTS** — five pure helpers handle it: `parseAmountToCents` (string→cents, never `round(parseFloat*100)`), `formatCents`, `splitEqualCents` (deterministic +1-cent remainder distribution in roster order so shares sum exactly), `computeBalances` (`{net, paid}`; invariant Σnet===0), `simplifyDebts` (greedy minimal settle-up). **Balances and the settle-up plan are DERIVED on every render, never stored** — deleting an expense just re-derives. The add-expense form is a native `<dialog id="exp-dialog">` with Equal/Custom modes; custom shares must sum to the total or Save is blocked. Expense descriptions are **user input** → `escapeHtml`. Permissions are owner-scoped: only `createdBy` deletes an expense; a settlement is recordable/undoable by either family involved (`from`/`to`); "Settle all" records `simplifyDebts` as `settlements` in one commit. `gateFamily(selectEl)` is the shared "pick your family first" gate.
