...
| `checklist.js` | Packing-list structure → `window.CHECKLIST` (+ `window.CHECKLIST_FLAT`). Also the Food tab's pre-set list → `window.FOOD` / `window.FOOD_FLAT`. Groups → subgroups → items, each item with flags. Plain script. |
...
| `app.js` | ES module. Imports `sync`, reads `window.TRIP_DATA` / `window.CHECKLIST`, renders all screens, wires nav + checklist interactions. Entry point (`boot()`). Registers the service worker. |
| `manifest.webmanifest` + `sw.js` + `icons/` | PWA: installable + offline. See the PWA note below. |
...
- Six tabs (Park / Do / Places / Pack / Food / Expenses) are `<section class="screen">` toggled via the `hidden` attribute by `switchTab()`. Bottom nav drives it (`.bottom-nav` is `grid-template-columns: repeat(6, 1fr)`).
...
- The **Food tab** (`renderFood`) reuses the claim system: it renders `window.FOOD_FLAT` (pre-set list) plus user-added `state.customFood` rows via `itemRowHtml`/`customItemRowHtml`. Its own `#food-family-select` writes the same shared `uiState.currentFamilyId` as Pack (both filled by `renderFamilySelects`). `handleClaim(row, { selectEl, warnDuplicates })` is shared between tabs — pass the tab's select and whether to warn on duplicate claims. Custom-item names are **user input** → escaped with `escapeHtml` before `innerHTML`. The remove (×) button only renders for items the current family added (`c.addedBy === currentFamilyId`).
- The **Expenses tab** (`renderExpenses`) is a Venmo/Splitwise-style splitter. **All money is INTEGER CENTS** — five pure helpers handle it: `parseAmountToCents` (string→cents, never `round(parseFloat*100)`), `formatCents`, `splitEqualCents` (deterministic +1-cent remainder distribution in roster order so shares sum exactly), `computeBalances` (`{net, paid}`; invariant Σnet===0), `simplifyDebts` (greedy minimal settle-up). Splitting amounts that don't divide evenly leaves unavoidable rounding "dust"; balances within `SETTLE_EPSILON_CENTS` (2¢) of zero are shown as **settled** and `simplifyDebts` drops sub-2¢ transfers, so the settle-up never shows silly `$0.01` rows. **Balances and the settle-up plan are DERIVED on every render, never stored** — deleting an expense just re-derives. The add-expense form is a native `<dialog id="exp-dialog">` with Equal/Custom modes; custom shares must sum to the total or Save is blocked. Expense descriptions are **user input** → `escapeHtml`. Permissions are owner-scoped: only `createdBy` deletes an expense; a settlement is recordable/undoable by either family involved (`from`/`to`); "Settle all" records `simplifyDebts` as `settlements` in one commit. `gateFamily(selectEl)` is the shared "pick your family first" gate.

### Sync (`sync.js`)
- State shape: `{ tripCode, families: [{id, name}], claims: { [itemId]: [familyId, ...] }, customFood: [{id, name, addedBy, addedAt}], expenses: [{id, description, amountCents, paidBy, shares:{famId:cents}, createdBy, createdAt}], settlements: [{id, from, to, amountCents, createdBy, createdAt}], updatedAt }`.
- A claim is a `familyId` in an item's array — multiple families can claim the same item.
- **Custom food items** (`state.customFood`) sync exactly like `claims` — touched in the same five spots (`freshState`, `readLocal`, `_pushToCloud`, the seed `setDoc`, the `onSnapshot` adopt block). `addCustomFood(name, familyId)` appends an item with id `custom.<ts>-<rand>` and auto-claims it; `removeCustomFood(id)` drops it and its claim. Both call `_commit()`.
- **`expenses` and `settlements`** (Expenses tab) sync the same way — same five touch points, money in **integer cents**. Mutators: `addExpense({description, amountCents, paidBy, shares, createdBy})`, `removeExpense(id)`, `addSettlement({from, to, amountCents, createdBy})`, `addSettlements(entries)` (batch — one commit, used by "Settle all"), `removeSettlement(id)`. Same whole-doc last-write-wins caveat as `claims` — two devices logging an expense in the same window can drop one (money makes it worse; no field-merge yet).
...
### PWA / offline (`sw.js`, `manifest.webmanifest`, `icons/`)
- The app is an installable PWA. `app.js` registers `sw.js` on `load` (relative path → correct scope at root or subpath). The service worker **only registers over `http(s)`/`localhost`** — not `file://`.
- `sw.js` precaches the app shell + local trail images (the `SHELL` list), serves navigations **network-first → cached `index.html`** offline, and everything else **stale-while-revalidate**. Live data (Firestore, the `firebasejs` SDK, Open-Meteo) is matched by `isLiveData()` and **never cached** — it goes straight to the network and degrades gracefully offline (sync stays local; forecast shows its fallback).
- **Gotcha — cache busting:** changing any shell file (html/css/js/icons) requires bumping `CACHE_VERSION` in `sw.js`, or installed clients keep serving the old cached shell. The `activate` handler deletes caches that don't match the current name.
- Icons are generated by `generate_icons.py` (pure stdlib, no Pillow) — re-run it to regenerate, but the script is not part of the deployed app.
