...
- Five tabs (Park / Do / Places / Pack / Food) are `<section class="screen">` toggled via the `hidden` attribute by `switchTab()`. Bottom nav drives it (`.bottom-nav` is `grid-template-columns: repeat(5, 1fr)`).
...
### Sync (`sync.js`)
- State shape: `{ tripCode, families: [{id, name}], claims: { [itemId]: [familyId, ...] }, customFood: [{id, name, addedBy, addedAt}], updatedAt }`.
