...
- Known limitation: it's still whole-document last-write-wins. If two devices edit *different* fields (one renames a family while another claims an item) within the same sync window, the later write wins the whole doc and can drop the other's change. Acceptable for this small-group, low-contention use; revisit with field-level merges if it becomes a problem.
...
- `DEFAULT_FAMILIES` has 7 slots; `normalizeFamilies` merges defaults additively so adding slots migrates old saved state without wiping renames.
